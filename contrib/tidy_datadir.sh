...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete NoirShares database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no NoirShares datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old NoirShares datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected NoirShares 0.7 datadir."
    ;;
  3)
    echo "Detected NoirShares pre-0.8 datadir."
    ;;
  4)
    echo "Detected NoirShares 0.8 datadir."
    ;;
esac
