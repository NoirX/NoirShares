...
The RPM spec file provided here is for NoirShares-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://noirshares.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[NoirShares git repository](https://github.com/noirshares/noirshares) but are not part of
...
Currently only `Source10` is used, for the example `noirshares.conf` file.
...
in the noirshares git repository. At present only an SVG version of the NoirShares
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the noirshares
...
needed when 0.12.1 is released, a proper fix is already in the NoirShares git
master branch.
...
idea to build NoirShares Core against a static version of that library compiled
...
The `noirshares.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `noirshares-server` package. At this point, most RPM based
...
The `noirshares-server` package should have SELinux support. How to properly do
...
NoirShares-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/noirshares/). He has been
packaging NoirShares for Fedora at least since 2012.
...
4. I renamed the `noirshares` package that contains the Qt GUI to `noirshares-core` as
that appears to be how the general population refers to it, in contrast to
`noirshares-xt` or `noirshares-classic`. I wanted to make sure the general population
