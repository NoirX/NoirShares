...
###   This script attempts to download the signature file SHA256SUMS.asc from noirshares.org
...
WORKINGDIR="/tmp/noirshares"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://noirshares.org/bin/"
VERSIONPREFIX="noirshares-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'noirshares-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the NoirShares Core binary release signing key installed?"
   fi
