...
*Setup instructions for a Gitian build of NoirShares using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the NoirShares
...
- [Building NoirShares](#building-noirshares)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-noirshares-host/README.md).
...
Clone the git repositories for noirshares and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/noirshares/noirshares
...
in the noirshares repository under 'Fetch and build inputs' to install sources which require
...
To build NoirShares (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the noirshares repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/noirshares/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/noirshares/noirshares
...
For example:
```bash
URL=https://github.com/laanwj/noirshares.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit noirshares=${COMMIT} --url noirshares=${URL} ../noirshares/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit noirshares=${COMMIT} --url noirshares=${URL} ../noirshares/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit noirshares=${COMMIT} --url noirshares=${URL} ../noirshares/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the noirshares git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../noirshares/contrib/gitian-descriptors/*|sort|uniq )
...
NRSPATH=/some/root/path/noirshares.git
SIGPATH=/some/root/path/noirshares-detached-sigs.git

./bin/gbuild --url noirshares=${NRSPATH},signature=${SIGPATH} ../noirshares/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/noirshares-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/noirshares-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/noirshares-osx-build.assert
```
...
[noirshares/gitian.sigs](https://github.com/noirshares/gitian.sigs/) repository, or if that's not possible create a pull
