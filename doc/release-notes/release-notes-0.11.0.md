NoirShares Core version 0.11.0 is now available from:

  <https://noirshares.org/bin/noirshares-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer NRS/kB fee to be
...
For example, add the following to `noirshares.conf`:
...
related to the blockchain in the noirshares system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows NoirShares Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that NoirShares Core will 
...
For an example script see [noirshares-submittx](https://github.com/laanwj/noirshares-submittx).
...
- #5334 `cf87536` libnoirsharesconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new noirshares-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only NoirSharesAmountField
- #5753 `0f44672` Add noirshares logo to about screen
...
- #5809 `46bfbe7` Add noirshares-cli man page
...
- #5849 `41259ca` contrib/init/noirsharesd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename NoirShares-Qt.app to NoirShares-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for noirshares-tx)
